...
class Effect2:
	...
	def __init__(self,pygame, main_dir, mixer, FPSCLOCK, FPS):
		self.FIRE = pygame.image.load('cyan.png')
		self.PIXEL = pygame.image.load('fire_yellow.png')
		self.FIRE_YELLOW = pygame.image.load('blue.png')
		self.main_dir = main_dir
		self.mixer = mixer
		self.FPSCLOCK = FPSCLOCK
		self.FPS = FPS
		file_path = os.path.join(self.main_dir,'data','dubstep.wav')
	...
	def show(self,DISPLAYSURF, hkinect, mouse_x, mouse_y, mouse_x_old, mouse_y_old,  particle_xysize,velocity, random_numbers, gkinect):
		...
		if bodies is not None:
			for i in range(0, gkinect.max_body_count):
				...
				joint0 = PyKinectV2.JointType_WristRight
				joint1 = PyKinectV2.JointType_ThumbRight
		...
		# Algorithm	
			...
			if particle_xysize[element][2] > 0:
				...
				while True:
					...
					particle_xysize[element][7] = (random.randint(1,100), random.randint(1,100), random.randint(1,100))
